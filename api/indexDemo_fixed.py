...
def extract_text_from_docx(path):
    ...
    def is_education_like_table(table):
        ...
        if not any(first_cells):
            return False
        # Check for education keywords in headers
...
def validate_ai_data(data):
    ...
    certs = [safe_str(c) for c in clamp_list(data.get("certifications", []), 5) if safe_str(c)]
    ...
    raw_skills = clamp_list(data.get("skills", []), 15)
    skills = []
...
7. certifications: short strings, one per item. Include ALL certifications, awards, and achievements mentioned.
...
FIELD: summary
  ...
  - Target length: approximately 15 words (acceptable range: 12-17 words).
...
FIELD: education
  ...
  - If a table is present, read EVERY row and identify the HIGHEST degree only.
  - If education is given as plain text, produce a single-item array with the highest degree.
  - Return ONLY the highest degree as a single-item array.
...
FIELD: certifications
  - Extract ALL certifications, awards, and achievements from the resume.
  - Look for sections titled "Achievements", "Certifications", "Awards", "Honors".
  - Include every item: "Certified MTA: Database Fundamentals (Microsoft, 2019)", "Manager's Choice Award", "2nd Prize - Hackathon 2018 (REVA University)", etc.
  - Return as a JSON array of strings.
  - Do NOT miss any certification or award mentioned.
...
{{
  ...
  "certifications": ["Cert 1", "Cert 2", "Award 1"],
  "experience": [
    {{
      "title": "Role Title",
...
def render_skills_tags_compact(slide, skills, left, top, panel_width_emu, max_height=None):
    ...
    # Track rows used for max_height enforcement
    rows_used = 0
    max_rows = 999  # Default: no limit

    if max_height is not None:
        max_rows = max(1, int((max_height - TAG_H) / row_h) + 1)
...
def create_resume_ppt(data, images=None):
    ...
    # =========================================================
    # NEW PRIORITY-BASED SECTION ORDERING
    # =========================================================
    # Define sections with their data, minimum height needed, and priority
    # Order: Summary > Skills > Education > Certifications > Core Strengths

    sections = [
        ("summary", data.get("summary"), Inches(0.4)),          # High priority
        ("skills", data.get("skills"), Inches(0.5)),            # High priority (can shrink)
        ("education", data.get("education"), Inches(0.3)),      # High priority
        ("certifications", data.get("certifications"), Inches(0.25)), # High priority
        ("core_strengths", data.get("core_strengths"), Inches(0.4)),  # Lower priority
    ]

    # Track which sections were rendered
    rendered_sections = set()
    sidebar_overflow = []

    for section_name, section_data, min_height in sections:
        # Check if we have enough space
        remaining_space = sidebar_bottom - sy

        if section_name == "summary":
            if section_data and remaining_space >= min_height:
                sy = sidebar_section_label(slide, "Summary", sx, sy, sw)
                summary_w_in = sw / 914400
                summary_h = estimate_text_height(section_data, 11, summary_w_in) + 0.05
                summary_h = max(summary_h, 0.25)
                summary_h_emu = Inches(min(summary_h, 1.0))

                # Ensure we don't exceed bottom
                available = sidebar_bottom - sy - Inches(0.08)
                summary_h_emu = min(summary_h_emu, available)

                if summary_h_emu > Inches(0.15):
                    add_textbox(
                        slide, section_data, sx, sy, sw, summary_h_emu,
                        font_size=11, bold=True, color=TEXT, align=PP_ALIGN.JUSTIFY
                    )
                    sy += summary_h_emu + Inches(0.08)
                    rendered_sections.add("summary")
            else:
                sidebar_overflow.append(("Summary", [section_data] if section_data else []))

        elif section_name == "skills":
            if section_data and len(section_data) > 0 and remaining_space >= min_height:
                sy = sidebar_section_label(slide, "Key Skills", sx, sy, sw)
                # Calculate remaining space for skills
                remaining_for_skills = sidebar_bottom - sy - Inches(0.10)
                max_skills_height = min(Inches(1.2), max(Inches(0.3), remaining_for_skills))
                sy = render_skills_tags_compact(slide, section_data, sx, sy, int(sw), max_height=max_skills_height)
                sy += Inches(0.06)
                rendered_sections.add("skills")
            else:
                sidebar_overflow.append(("Key Skills", section_data if section_data else []))

        elif section_name == "education":
            if section_data and len(section_data) > 0 and remaining_space >= min_height:
                sy = sidebar_section_label(slide, "Education", sx, sy, sw)
                for edu_entry in section_data:
                    if sy + Inches(0.20) > sidebar_bottom - Inches(0.05):
                        break
                    entry_h = Inches(max(estimate_text_height(edu_entry, 11, sw / 914400) + 0.04, 0.20))
                    # Ensure we don't exceed bottom
                    available = sidebar_bottom - sy - Inches(0.02)
                    entry_h = min(entry_h, available)
                    if entry_h > Inches(0.1):
                        add_textbox(slide, f"• {edu_entry}", sx, sy, sw, entry_h,
                                   font_size=11, bold=True, color=TEXT)
                        sy += entry_h + Inches(0.02)
                sy += Inches(0.06)
                rendered_sections.add("education")
            else:
                sidebar_overflow.append(("Education", section_data if section_data else []))

        elif section_name == "certifications":
            if section_data and len(section_data) > 0 and remaining_space >= min_height:
                sy = sidebar_section_label(slide, "Certifications", sx, sy, sw)
                for cert in section_data:
                    if sy + Inches(0.20) > sidebar_bottom - Inches(0.05):
                        break
                    # Ensure we don't exceed bottom
                    available = sidebar_bottom - sy - Inches(0.02)
                    cert_h = min(Inches(0.20), available)
                    if cert_h > Inches(0.1):
                        add_textbox(slide, f"• {cert}", sx, sy, sw, cert_h,
                                   font_size=11, bold=True, color=TEXT)
                        sy += Inches(0.21)
                sy += Inches(0.06)
                rendered_sections.add("certifications")
            else:
                sidebar_overflow.append(("Certifications", section_data if section_data else []))

        elif section_name == "core_strengths":
            if section_data and len(section_data) > 0 and remaining_space >= min_height:
                sy = sidebar_section_label(slide, "Core Strengths", sx, sy, sw)
                # Calculate remaining space for strengths
                remaining_for_strengths = sidebar_bottom - sy - Inches(0.10)
                max_strengths_height = min(Inches(0.8), max(Inches(0.3), remaining_for_strengths))
                sy = render_skills_tags_compact(slide, section_data, sx, sy, int(sw), max_height=max_strengths_height)
                sy += Inches(0.06)
                rendered_sections.add("core_strengths")
            else:
                sidebar_overflow.append(("Core Strengths", section_data if section_data else []))
    ...
    # -- HANDLE SIDEBAR OVERFLOW IN RIGHT PANEL --
    if sidebar_overflow:
        for title, items in sidebar_overflow:
            if not items or (isinstance(items, list) and len(items) == 0):
                continue
            if ry + Inches(0.5) > rp_bottom:
                break
            bar_bottom = section_header_bar(slide, title, rx, ry, rw)
            ...
            for item in items:
                if not isinstance(item, str):
                    item = str(item)
                if ry + Inches(0.20) > rp_bottom:
                    break
                item_h = Inches(max(estimate_text_height(item, 11, rw / 914400) + 0.04, 0.20))
                add_textbox(slide, f"• {item}", rx + Inches(0.04), ry, rw - Inches(0.08), item_h,
                           font_size=11, bold=True, color=TEXT)
                ry += item_h + Inches(0.02)
            ry += Inches(0.10)
    ...
    if data["experience"]:
        if use_exp_table:
            ...
            if data["projects"] and not use_proj_table:
                ...
                exp_bottom = rp_bottom - proj_reserve - Inches(0.10)
            else:
